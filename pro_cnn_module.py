...
Xt_train=torch.from_numpy(X_train).permute(0,2,1)
Yt_train=torch.from_numpy(Y_train)
Xt_test=torch.from_numpy(X_test).permute(0,2,1)
...
(y,)
'''
# transform the data for cnn
Xt_train = Variable(torch.unsqueeze(Xt_train, dim=1))
Xt_test = Variable(torch.unsqueeze(Xt_test, dim=1))
print(Xt_train.shape)
print(Xt_test.shape)
'''
output:
...
# [8] CNN
# Hyper parameters
epochs=7     # how many times we train the whole data
batch_size = 5 # train 1 each
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(         # input shape (1, 9, 50)
            nn.Conv2d(
                in_channels=1,              # input height (1 means simple color)
                out_channels=16,            # n_filters
                kernel_size=3,              # filter size
                stride=1,                   # filter movement/step
                padding=1,                  # if want same width and length of this image after con2d, padding=(kernel_size-1)/2 if stride=1
            ),                              # output shape (16, 9, 50)
            nn.ReLU(),                      # activation
            nn.MaxPool2d(kernel_size=1),    # choose max value in 1x1 area, output shape (16, 9, 50)
        )
        self.conv2 = nn.Sequential(         # input shape (16, 9, 50)
            nn.Conv2d(16, 32, 3, 1, 1),     # output shape (32, 9, 50)
            nn.ReLU(),                      # activation
            nn.MaxPool2d(1),                # output shape (32, 9, 50)
        )
        self.out = nn.Linear(32 * 9 * 50, 4)   # fully connected layer, output 4 classes

    def forward(self, x):
        x = self.conv1(x)
        x = self.conv2(x)
        x = x.view(x.size(0), -1)           # flatten the output of conv2 to (batch_size, 32 * 9 * 50)
        output = self.out(x)
...
for epoch in range(epochs):
    for step in range(len(Xt_train)//batch_size):
        ...
        output,_ = cnn(b_x.float())               # cnn output
        ...
        if step % 20 == 0:
            test_out,_ = cnn(Xt_test.float())  # (sample,time_step,input_size)
...
test_output,_ = cnn(Xt_test[:10].float())
